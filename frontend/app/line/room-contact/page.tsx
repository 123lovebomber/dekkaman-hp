...
//     );
// }
// app/line/room-contact/page.tsx
import { redirect } from "next/navigation";

type RoomContactPageProps = {
    searchParams?: {
...
// LINE公式アカウントのベーシックID（今回使うのは @dekkaman）
const OA_BASIC_ID_PREMIUM = "@dekkaman";

export default function RoomContactPage({ searchParams }: RoomContactPageProps) {
    const buildingName = searchParams?.building_name ?? "";
    const roomNo = searchParams?.room_no ?? "";
    // room_id は現状テキストには使わないが、将来用に受け取っておく
    const _roomId = searchParams?.room_id ?? "";

    // building_name / room_no が取れなかった場合は簡単なエラー画面を返す
    if (!buildingName || !roomNo) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-zinc-50 px-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-md p-6">
                    <h1 className="text-lg font-semibold mb-3">LINE問い合わせエラー</h1>
                    <p className="text-sm text-zinc-700 mb-3">
                        物件名または部屋番号の情報が取得できませんでした。
                    </p>
                    <p className="text-xs text-zinc-500 leading-relaxed">
                        恐れ入りますが、物件ページからもう一度「LINEで問い合わせる」ボタンを押してお試しください。
                    </p>
                </div>
            </div>
        );
    ...
    // 表示用ラベル（例：レジディア白金高輪0101号室）
    const label = `${buildingName}${roomNo}号室`;

    // LINEトークの入力欄にプリセットしたい文章
    const messageText = `${label}の割引後の初期費用をお願いします。`;

    // OAベーシックIDとテキストをURLエンコードして deeplink を作成
    const encodedBasicId = encodeURIComponent(OA_BASIC_ID_PREMIUM);
    const encodedMessage = encodeURIComponent(messageText);

    const lineUrl = `https://line.me/R/oaMessage/${encodedBasicId}/?${encodedMessage}`;

    // 即座に LINE アプリ側（またはブラウザの LINE 画面）へリダイレクト
    redirect(lineUrl);
}